...
function updateHolidayPrep() {
  ...
  var today = new Date();
  let thisMonth = Utilities.formatDate(today, Session.getScriptTimeZone(), "MMMMM");
  ...
  if(thisMonth != "January" && annualResetDone) {
   var importRow = holidayPrep.getRowNumber(holidayPrep.nameColumnName, holidayPrep.annualResetRowTaskName);
   holidayPrep.setValue(holidayPrep.checkboxColumn, importRow, false);
  }

  var importMonthColumn = holidayPrep.getColumnNumber(holidayPrep.importMonthColumnName);
  var importTaskColumn = holidayPrep.getColumnNumber(holidayPrep.nameColumnName);
  var importMonth;
  var taskName;
...
function annualReset() {
  resetAllImported();
  updateDueDates();
  var annualResetRow = holidayPrep.getRowNumber(holidayPrep.nameColumnName,holidayPrep.annualResetRowTaskName);
...
function resetAllImported() {
  ...
  holidayPrep.sheet.getRange(2, importedColumn, holidayPrep.lastRow, 1).setValue(false);
}
...
function updateDueDates() {
  ...
  var occasionColumn = holidayPrep.getColumnNumber(holidayPrep.projectColumnName);
  var dueDateColumn = holidayPrep.getColumnNumber(holidayPrep.dueDateColumnName);
...
function updateDueDateYear(row) {
  ...
  var newDate = new Date(currentYear, originalDate.getMonth(), originalDate.getDate()); 
...
function getMothersDay(year) {
  ...
  firstSunday.setDate(mayFirst.getDate() + (7 - (dayOfWeek === 0 ? 7 : dayOfWeek))); //Corrected to handle if May 1st is a Sunday
...
function getFathersDay(year) {
  ...
  firstSunday.setDate(juneFirst.getDate() + (7 - (dayOfWeek === 0 ? 7 : dayOfWeek)));
