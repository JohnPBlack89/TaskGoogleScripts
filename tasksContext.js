...
class tasksContext extends sheetContext {
  ...
  importSpreadsheet(spreadsheet) {
    if (Object.prototype.toString.call(spreadsheet) === "[object Spreadsheet]")
      throw new Error("Must pass a Spreadsheet object to function importSpreadsheetToTask");

    const existingNames = spreadsheet.getSheets().map(sheet => sheet.getName());
    const taskSheetName = taskSheetNames.filter(name => existingNames.includes(name));
  ...
  importSheet(sheet) {
    if(typeof sheet == "string")
      sheet = new tasksContext(sheet,1);

    if(!(sheet instanceof tasksContext))
      throw new Error("Must pass either a string or tasksContext object to function importSheet(sheet)" );
    ...
    for(var i = sheet.titleRow + 1; i < sheet.lastRow; i++){
      ...
      if(checkboxField != false && dueDate - today > sheet.projectPrepDaysNeeded)
        sheet.importRow(sheet, i)
    }
  }

  importGoogleDoc(url) {
  var table = getGoogleDocTable(
    url,
    tableNumberInTab, 
    tableTabName);
  ...
  importRow(sheet,externalSheetRowNumber) {
    if(!(sheet instanceof sheetContext))
        throw new Error("Must pass a sheetContext object to function importRow(sheet)" );

    var internalRowNumber = this.lastRow + 1;

    // Migrate Task Name
    this.importColumn(sheet,externalSheetRowNumber, internalRowNumber, this.nameColumnName);
    this.importColumn(sheet,externalSheetRowNumber, internalRowNumber, this.dueDateColumnName);
    this.importColumn(sheet,externalSheetRowNumber, internalRowNumber, this.projectColumnName);
    this.importColumn(sheet,externalSheetRowNumber, internalRowNumber, this.genreColumnName);
    this.importColumn(sheet,externalSheetRowNumber, internalRowNumber, this.notesColumnName);

    // Mark column as imported
    if (this.checkboxColumnName == null)
      return;

    var checkboxColumnNumber = this.getColumnNumber(this.checkboxColumnName);
    this.sheet.getRange(externalSheetRowNumber,checkboxColumnNumber).setValue(true);
  }
  ...
  importColumn(sheetContext,externalRowNumber, internalRowNumber, columnName) {
    ...
    var externalColumnNumber= sheetContext.getColumnNumber(sheetContext[columnName]);
    if(internalColumnNumber == null || externalColumnNumber == null)
      return;
    migrateCell(sheetContext, externalColumnNumber, externalRowNumber, this.sheet, internalColumnNumber, internalRowNumber);
  }
   ...
   */
  sortTasks() {
    var tasksTable = this.sheet.getRange(this.titleRow + 1, 1, this.lastRow - this.titleRow + 1, this.lastColumn);
  ...
  getGenreNamedRangeHyperLinks() {
    ...
    for(var i = 1; i < totalRows; i++){
      getNamedRangeHyperlinksForCell(this.Sheet.getName(), i, genreColumnNumber, genreNamedRangeName);
    }
  }
  ...
  highlightDates() {
    var dueDateColumn = this.getColumnNumber(this.dueDateColumnName);
    var totalRows = this.lastRow - this.titleRow;
    var range = this.sheet.getRange(this.titleRow + 1, dueDateColumn, totalRows, 1);
    ...
    for (var i = 0; i < totalRows; i++) {
      ...
      if(cellDate == null || cellDate == 0) {
        this.sheet.getRange(i + 1 + this.titleRow,1,1,this.lastColumn).setBackground("#434343");
        continue;
      }
      ...
      if (diffDays < 0) {
        cell.setBackground(this["pastDateBackgroundColor" + addition.toString()]); // Red for past dates
      } else if (diffDays === 0) {
        cell.setBackground(this["todayBackgroundColor" + addition.toString()]); // Yellow for today
      } else if (diffDays <= nearDateDaysAhead) {
        cell.setBackground(this["nearDateBackgroundColor" + addition.toString()]); // Green for dates within a week
      } else {
